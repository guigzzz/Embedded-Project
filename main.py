...
i2c.writeto_mem(SLAVEADDRPROX,COMMAND,b'\x07') #enable periodic prox measurement
time.sleep(0.1)
i2c.writeto_mem(SLAVEADDRPROX,PROXIMITYRATE,b'\x02') #set prox measurement rate to 7sample/sec
time.sleep(0.1)
...
def gethumdandtemp():
	i2c.writeto(SLAVEADDRTEMP,HUMDANDTEMP)
	time.sleep(0.1)
	humd = i2c.readfrom(SLAVEADDRTEMP,2)
	i2c.writeto(SLAVEADDRTEMP,GETTEMP)
	time.sleep(0.1)
...
while 1:
	...
	print('proximity: %d, ambient light: %d, humidity: %d %%, temperature: %d C, duty cycle: %d '%(prox,amb,humd,temp, led_duty))
